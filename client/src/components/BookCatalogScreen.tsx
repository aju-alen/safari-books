...
export type BookCatalogScreenProps = {
  categoryKey?: string;
  /** When false, hides back button (e.g. root Search tab). */
  showBackButton?: boolean;
  /** Header title when there is no category (default: "All Audiobooks"). */
  screenTitle?: string;
...
const BookCatalogScreen = ({
  ...
  useEffect(() => {
    ...
    const filtered = books.filter((book) => {
      ...
      const rawCat = book.category;
      const catStr = Array.isArray(rawCat)
        ? rawCat.join(' ').toLowerCase()
        : String(rawCat ?? '').toLowerCase();
      return title.includes(q) || author.includes(q) || catStr.includes(q);
    });
    ...
    try {
      ...
      if (categoryKey) {
        response = await axiosWithAuth.get(
          `${ipURL}/api/listeners/get-all-books-data/${categoryKey}`,
      ...
      <View style={styles.bookCardHeader}>
        <Image
          source={{
            uri:
              item.coverImage ||
              'https://via.placeholder.com/150x200/4F46E5/FFFFFF?text=Book',
          }}
  ...
  const renderEmptyState = () => (
    <View style={styles.emptyState}>
      <View
        style={[styles.emptyStateIcon, { backgroundColor: `${theme.primary}15` }]}
      >
  ...
  const headerTitle = categoryKey
    ? `${categoryLabel(categoryKey)} Books`
    : screenTitle ?? 'All Audiobooks';
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        <View style={styles.headerLeft}>
          ...
          <Text
            style={[styles.headerTitle, !showBackButton && { marginLeft: 0 }]}
            numberOfLines={1}
          >
            {headerTitle}
          </Text>
